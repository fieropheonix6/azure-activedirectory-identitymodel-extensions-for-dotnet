    ...
    public partial class JsonWebTokenHandler : TokenHandler
    {
            ...
            int tokenLifetimeInMinutes)
        {
            ...
            try
            {
                ...
                writer.Reset();
                WritePayloadClaims(ref writer, tokenDescriptor, setdefaultTimesOnTokenCreation, tokenLifetimeInMinutes);

                int payloadLength = buffer.WrittenCount - payloadStart;

                // Span of base64UrlEncodedHeader + "." + base64UrlEncodedPayload
                int signatureSize = 0;
                if (tokenDescriptor.SigningCredentials != null)
                    signatureSize = SupportedAlgorithms.MaximumSignatureSizeInBytes(tokenDescriptor.SigningCredentials.Algorithm);
                ...
                int sizeOfEncodedHeader = Base64UrlEncoder.Encode(buffer.WrittenSpan.Slice(0, headerLength), encodedBuffer.AsSpan());
                // encodedBuffer => EncodedHeader

                encodedBufferSpan[sizeOfEncodedHeader] = '.';
                // encodedBuffer => EncodedHeader.

                int sizeOfEncodedPayload = Base64UrlEncoder.Encode(buffer.WrittenSpan.Slice(payloadStart, payloadLength), encodedBuffer.AsSpan(sizeOfEncodedHeader + 1));
                // encodedBuffer => EncodedHeader.EncodedPayload

                int sizeOfEncodedHeaderAndPayload = sizeOfEncodedHeader + sizeOfEncodedPayload + 1;
                encodedBuffer[sizeOfEncodedHeaderAndPayload] = '.';
                // encodedBuffer => EncodedHeader.EncodedPayload
                ...
                int sizeOfHeaderAndPayloadBytes = (int)Encoding.UTF8.GetBytes(encodedBuffer.AsSpan(0, sizeOfEncodedHeaderAndPayload), buffer);
                ...
                if (tokenDescriptor.SigningCredentials != null)
                {
                    ...
                    sizeOfEncodedSignature = Base64UrlEncoder.Encode(signature.AsSpan().Slice(0, signatureBytes), encodedBuffer.AsSpan(sizeOfEncodedHeaderAndPayload + 1));
                }
                ...
                if (tokenDescriptor.EncryptingCredentials != null)
                {
                    ...
                    Encoding.UTF8.GetBytes(encodedBuffer.AsSpan(sizeOfEncodedHeaderAndPayload + 1, sizeOfEncodedSignature), buffer);
                    ...
                    return encodedBuffer.AsSpan(0, sizeOfEncodedHeader + 2 + sizeOfEncodedPayload + sizeOfEncodedSignature).ToString();
            }
            finally
            {
            ...
            int tokenLifetimeInMinutes)
        {
            ...
            using (MemoryStream utf8ByteMemoryStream = new())
            {
                ...
                try
                {
                    ...
                    int encodedBufferSize = ((int)utf8ByteMemoryStream.Length + 4 + signatureSize) / 3 * 4;
                    encodedBuffer = ArrayPool<char>.Shared.Rent(encodedBufferSize);
                    Span<char> encodedBufferSpan = encodedBuffer.AsSpan();
                    ...
                    // Need Utf8Bytes of 'EncodedHeader.EncodedPayload' which is used to calculate the signature
                    int maxUtf8Bytes = Encoding.UTF8.GetMaxByteCount(encodedBufferSize);
                    ...
                    if (tokenDescriptor.SigningCredentials != null)
                    {
                        byte[] signature = JwtTokenUtilities.CreateEncodedSignature(utf8Bytes, 0, sizeOfEncodedHeaderAndPayloadUtf8Bytes, tokenDescriptor.SigningCredentials);
                    ...
                    // EncodedHeader | . | EncodedPayload | . | EncodedSignature (note: signature may be null)
                    if (tokenDescriptor.EncryptingCredentials != null)
                    {
                        string inner = Encoding.UTF8.GetString(utf8ByteMemoryStream.GetBuffer());
                        return EncryptToken(
                            utf8Bytes.AsSpan().Slice(0, sizeofJws).ToArray(),
                            tokenDescriptor.EncryptingCredentials,
                            tokenDescriptor.CompressionAlgorithm,
                            tokenDescriptor.AdditionalHeaderClaims,
                            tokenDescriptor.TokenType);
        ...
        internal static void WritePayloadClaims(ref Utf8JsonWriter writer, SecurityTokenDescriptor tokenDescriptor, bool setDefaultTimesOnTokenCreation, int tokenLifetimeInMinutes)
        {
